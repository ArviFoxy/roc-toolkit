...
void Reporter::process_stream_snapshot_block(const header::XrPacket& xr,
                                             const header::XrStreamSnapshotBlock& blk) {
    roc_panic_if_msg(report_state_ != State_Processing,
                     "rtcp reporter: invalid call order");

    // SSRC from XR is stream receiver (RTCP packet originator).
    // SSRC from Stream Snapshot block is stream sender (RTCP packet recipient).
    const packet::stream_source_t recv_source_id = xr.ssrc();
    const packet::stream_source_t send_source_id = blk.ssrc();
    ...
    core::SharedPtr<Stream> stream = find_stream_(recv_source_id, NoAutoCreate);
    if (!stream || !stream->has_remote_recv_report) {
        // Ignore Stream Snapshot if there was no matching SR.
        return;
    }

    roc_log(
        LogTrace,
        "rtcp reporter: processing Stream Snapshot block: send_ssrc=%lu recv_ssrc=%lu",
        (unsigned long)send_source_id, (unsigned long)recv_source_id);
    ...
    for (size_t n = 0; n < blk.n_entries() && n_out < MaxStreamSnapshots; n++) {
        ...
        if (e.has_niq_instant()) {
            snap.niq_instant = packet::ntp_2_nanoseconds(e.niq_instant());
        }
        if (e.has_niq_mean()) {
            snap.niq_mean = packet::ntp_2_nanoseconds(e.niq_mean());
        }
        if (e.has_e2e_latency()) {
            snap.e2e_latency = packet::ntp_2_nanoseconds(e.e2e_latency());
        }
        if (e.has_warp()) {
            snap.has_warp = true;
            snap.warp_ppb = e.warp_ppb();
        }
...
// Generate XR Stream Snapshot block to deliver to remote sender.
void Reporter::generate_stream_snapshot_block(size_t addr_index,
                                              size_t stream_index,
                                              header::XrStreamSnapshotBlock& blk,
                                              header::XrStreamSnapshotEntry* entries,
                                              size_t& n_entries) {
    roc_panic_if_msg(!is_receiving(),
                     "rtcp reporter: Stream Snapshot can be generated only by receiver");

    Stream* stream = address_index_[addr_index]->recv_stream_index[stream_index];
    roc_panic_if(!stream);

    const RecvReport& report = *stream->local_recv_report;
    ...
    for (size_t n = 0; n < report.n_snapshots && n < MaxStreamSnapshots; n++) {
        ...
        e.reset();
        e.set_grid_index(snap.grid_index);
        e.set_position(snap.position);
        if (snap.niq_instant >= 0) {
            e.set_niq_instant(packet::nanoseconds_2_ntp(snap.niq_instant));
        }
        if (snap.niq_mean >= 0) {
            e.set_niq_mean(packet::nanoseconds_2_ntp(snap.niq_mean));
        }
        if (snap.e2e_latency >= 0) {
            e.set_e2e_latency(packet::nanoseconds_2_ntp(snap.e2e_latency));
        }
        if (snap.has_warp) {
            e.set_warp_ppb(snap.warp_ppb);
        }
